...
[ -d ${stage}/usr/portage ] || install -d ${stage}/usr/portage
[ -d ${stage}/etc/portage ] || install -d ${stage}/etc/portage
[ -d ${rfs}/var/tmp/native ] || install -d ${rfs}/var/tmp/native
[ -d ${rfs}/var/tmp/cross ] || install -d ${rfs}/var/tmp/cross
[ -d ${stage}/var/tmp/native ] || install -d ${stage}/var/tmp/native
...
mount -o bind ${stage} ${sysroot}
mount -o bind ${portage} ${sysroot}/usr/portage
mount -o bind ${target_portage} ${sysroot}/etc/portage
...
mount -o bind ${native_pkgs} ${sysroot}/var/tmp/native
...
[ -L ${target_base}/etc/make.conf ] || ln -s portage/make.conf-${arch} ${target_base}/etc/make.conf
[ -L ${target_base}/etc/make.profile ] || ln -s ../usr/portage/profiles/default/linux/${arch}/${base} ${target_base}/etc/make.profile
export STAGEDIR="${target_base}"
...
umount ${sysroot}/var/tmp/native
umount ${rfs}/var/tmp/cross/binpkgs
umount ${rfs}/var/tmp/cross
umount ${rfs}/var/tmp/native
umount ${sysroot}/etc/portage
umount ${sysroot}/usr/portage
umount ${sysroot}
